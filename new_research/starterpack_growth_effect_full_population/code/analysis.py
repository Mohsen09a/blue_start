...
class StarterPackGrowthConfig:
    # None means the complete eligible treated population; no treated-user sample.
    treatment_rows: int | None = None
...
def nearest_propensity_matches(
    ...
    *,
    ...
    # Sort controls once by (stratum, logit). The sampled implementation could
    # afford rescanning the full array for every stratum; the full-population
    # run cannot, because that would create billions of repeated comparisons.
    control_indexes_all = np.flatnonzero(~treated)
    order = np.lexsort((logits[control_indexes_all], strata[control_indexes_all]))
    sorted_indexes = control_indexes_all[order]
    sorted_strata = strata[sorted_indexes]
    boundaries = np.flatnonzero(sorted_strata[1:] != sorted_strata[:-1]) + 1
    starts = np.concatenate([np.asarray([0]), boundaries])
    ends = np.concatenate([boundaries, np.asarray([len(sorted_indexes)])])
    control_groups: dict[Any, tuple[np.ndarray, np.ndarray]] = {
        sorted_strata[start].item(): (
            logits[sorted_indexes[start:end]],
            sorted_indexes[start:end],
        )
...
def _insert_match_rows(
    ...
    treated_indexes: np.ndarray,
    control_indexes: np.ndarray,
    ...
    # Build only one insertion batch at a time. A million Python tuples would
    # otherwise consume hundreds of megabytes in addition to the NumPy arrays.
    for start in range(0, len(treated_indexes), 20_000):
        end = min(start + 20_000, len(treated_indexes))
        rows = [
            (
                match_id,
                int(obs_ids[treated_index]),
                int(obs_ids[control_index]),
                float(propensity[treated_index]),
                float(propensity[control_index]),
                float(distance),
            )
            for match_id, treated_index, control_index, distance in zip(
                range(start + 1, end + 1),
                treated_indexes[start:end],
                control_indexes[start:end],
                distances[start:end],
                strict=True,
            )
        ]
...
def run_starterpack_growth_study(
    ...
    if config.treatment_rows is not None and config.treatment_rows < 100:
    ...
    with connect(settings) as con:
        ...
        treatment_limit = (
            "" if config.treatment_rows is None else f"LIMIT {config.treatment_rows}"
        )
        con.execute(
            f"""
        ...
        _stage(stages, "treatment_dates_full_population", stage_start)
        ...
        if control_count * config.max_control_reuse < treatment_count:
            raise RuntimeError(
                f"too few reusable controls ({control_count}) for {treatment_count} treatments"
            )
        con.execute(
            """
        ...
        population_counts = {
            ...
            "analyzed_treated_users": treatment_count,
    ...
    summary = {
        ...
        "limitations": [
            ...
            "The analysis includes the complete eligible treated population; control candidates are deterministically bounded to eight per treated user before eligibility filtering.",
